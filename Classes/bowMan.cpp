...
void bowMan::object_select(Vector<AllSprites *> sprites){//判定攻击目标
...
void bowMan::attack_ani(){//攻击
	...
	if (m_Sprite->getAnchorPoint().x != 0.125&&m_Sprite->getAnchorPoint().y != 0.125){//此段代码解决了点击普攻闪烁问题
		...
		m_Sprite = Sprite::createWithSpriteFrameName("B2f_0.png");//恢复精灵原来的贴图样子
		m_Sprite->setFlipX(SpriteDirecton);
...
void bowMan::attack_A_effect(AllSprites *enemy){//溅血效果
	hittest_A = false;
}
void bowMan::blood_effect(AllSprites *sprite){//溅血效果
	Vector<SpriteFrame* > cache;//缓冲池
	char txt[50] = {};
	int i;
	for (i = 0; i <= 4; i++)
	...
	Action * act = Sequence::create(
		...
		CCCallFuncN::create(sprite->get_n_Sprite(), callfuncN_selector(bowMan::removeself)), //删除自己
		NULL);
...
void bowMan::removeself(Node *pSender){//从父类移除图片特效
	pSender->removeFromParentAndCleanup(true);
}
void bowMan::move(){//朝着目标移动
	isMoving = true;
...
void bowMan::SetAnimation(int from, int to, bool run_directon, int loop){//动画
...
void bowMan::StopAnimation(){//停止所有动画
	if (!IsRunning)
		return;
	m_Sprite->stopAllActions();//当前精灵停止所有动画  
	//恢复精灵原来的初始化贴图   
