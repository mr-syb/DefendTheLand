...
void cavMan::object_select(Vector<AllSprites *> sprites){//判定攻击目标
...
void cavMan::attack_ani(){//攻击
	...
	if (m_Sprite->getAnchorPoint().x != 0.125&&m_Sprite->getAnchorPoint().y != 0.125){//此段代码解决了点击普攻闪烁问题
		...
		m_Sprite = Sprite::createWithSpriteFrameName("B3f_0.png");//恢复精灵原来的贴图样子
		m_Sprite->setFlipX(SpriteDirecton);
	...
	IsRunning = true;
	CCCallFunc* callFunc = CCCallFunc::create(this, callfunc_selector(cavMan::attack_A_img_end));
	CCActionInterval* attackact = CCSequence::create(act, callFunc, NULL);//序列动作
	m_Sprite->runAction(attackact);
}
void cavMan::attack_A_img_end(){
	...
	//将动画包装成一个动作  
	Animate* act = Animate::create(animation);
	CCCallFunc *callFunc = CCCallFunc::create(CC_CALLBACK_0(cavMan::attack_A_end, this));
	CCActionInterval* attackact = CCSequence::create(act, callFunc, NULL);//序列动作
	m_Sprite->runAction(attackact);
}
void cavMan::attack_A_end(){//停止攻击
	if (SpriteDirecton){//移动间攻击
...
void cavMan::attack_A_effect(AllSprites *enemy){//溅血效果
	SimpleAudioEngine::getInstance()->stopEffect(SimpleAudioEngine::getInstance()->playEffect(wave_A));
	SimpleAudioEngine::getInstance()->playEffect(cavMan_hit);
	hittest_A = false;
	blood_effect(enemy);
	//地方单位掉血
	enemy->set_life(-1 * damage_of_A);
}
void cavMan::blood_effect(AllSprites *sprite){//溅血效果
	Vector<SpriteFrame* > cache;//缓冲池
	char txt[50] = {};
	int i;
	for (i = 0; i <= 4; i++)
	...
	Action * act = Sequence::create(
		...
		CCCallFuncN::create(sprite->get_n_Sprite(), callfuncN_selector(cavMan::removeself)), //删除自己
		NULL);
...
void cavMan::removeself(Node *pSender){//从父类移除图片特效
	pSender->removeFromParentAndCleanup(true);
}
void cavMan::move(){//朝着目标移动
	isMoving = true;
...
void cavMan::deadth_ani(){
	...
	IsRunning = true;
	CCCallFunc* callFunc = CCCallFunc::create(this, callfunc_selector(cavMan::setisOver));
	CCActionInterval* deathact = CCSequence::create(act, fadeout, callFunc, NULL);//序列动作
	m_Sprite->runAction(deathact);
}
void cavMan::setisOver(){
	isOver = true;
}
void cavMan::SetAnimation(int from, int to, bool run_directon, int loop){//动画
...
void cavMan::StopAnimation(){//停止所有动画
	if (!IsRunning)
		return;
	m_Sprite->stopAllActions();//当前精灵停止所有动画  
	//恢复精灵原来的初始化贴图   
...
void cavMan::walk(bool run_directon){
	this->SetAnimation(walk_from, walk_to, run_directon, -1);
}

bool cavMan::get_skill_J_ishurted(){
	return skill_J_ishurted;
}
void cavMan::set_skill_J_ishurted(bool skill_J_ishurted){
	this->skill_J_ishurted = skill_J_ishurted;
}
bool cavMan::get_skill_K_ishurted(){
	return skill_K_ishurted;
}
void cavMan::set_skill_K_ishurted(bool skill_K_ishurted){
	this->skill_K_ishurted = skill_K_ishurted;
}
int cavMan::get_life(){
	return life;
}
void cavMan::set_life(int delta){
	life += delta;
	auto act = ScaleTo::create(0.2f, life / (float)orginal_life*0.4, 0.7);
	life_bar_fore->runAction(act);
}
void cavMan::set_life_bar(int side){
	if (side == 0){
...
bool cavMan::get_hittest_A(){
	return hittest_A;
}
void cavMan::set_hittest_A(bool hittest_A){
	this->hittest_A = hittest_A;
}
bool cavMan::get_Direction(){
	return SpriteDirecton;
}
bool cavMan::get_isblowup(){
	return isblowup;
}
void cavMan::set_isblowup(bool isblowup){
	this->isblowup = isblowup;
}
bool cavMan::get_canControl(){
	return canControl;
}
void cavMan::set_canControl(bool canControl){
	this->canControl = canControl;
}
bool cavMan::get_isOver(){
	return isOver;
}
bool cavMan::get_isdeath(){
	return isdeath;
}
int cavMan::get_typeid(){
	return type_id;
}
int cavMan::get_attack_A_distance(){
	return attack_area;
}
bool cavMan::is_melee(){
	return is_melee_unit;
}
float cavMan::get_speed(){
	return speed;
}
int cavMan::earn_money(){
	return pay_back;
}
Sprite* cavMan::get_n_Sprite(){
	return	n_Sprite;
}
void cavMan::set_money_paid(bool paid){
	this->paid = paid;
}
bool cavMan::get_money_paid(){
	return paid;
}
void cavMan::set_isMoving(bool isMoving){
	this->isMoving = isMoving;
}
bool cavMan::get_isMoving(){
	return isMoving;
}