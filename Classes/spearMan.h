...
USING_NS_CC;
class spearMan :public public_mercenary{
public:
	spearMan(int side);
	~spearMan();
	...
	void walk(bool run_directon);//走路动画
	void idle();//站立动画
	...
	int attack_A_distance;//平A增加的范围
	int attack_area;
	int attack_gap = 41;//普攻间隔
	int attack_now = 0;//进行多长时间的移动后可以立刻攻击
	float ox = 0;//复原x坐标
	void deadth_ani();//死亡动画
	bool isdeath = false;//是否死亡
	void setisOver();//设置可以移除
	int earn_money()override;//值多少钱
	...
	int get_attack_A_distance();//平A距离
