...
pikeMan::pikeMan(int side){
	...
	orginal_speed = speed;
	damage_of_A = 12;
	attack_A_distance = 48;//平A距离
	attack_area = 48 + 18;
	this->side = side;
	sprite_name = "B4f_6.png";//初始图片
	name_each = "B4f_";
	walk_from = 4;
	walk_to = 5;
	attack_from = 0;
...
void pikeMan::object_select(Vector<AllSprites *> sprites){//判定攻击目标
...
void pikeMan::attack_ani(){//攻击
	...
	if (m_Sprite->getAnchorPoint().x != 0.125&&m_Sprite->getAnchorPoint().y != 0.125){//此段代码解决了点击普攻闪烁问题
		...
		m_Sprite = Sprite::createWithSpriteFrameName("B4f_0.png");//恢复精灵原来的贴图样子
		m_Sprite->setFlipX(SpriteDirecton);
	...
	IsRunning = true;
	CCCallFunc* callFunc = CCCallFunc::create(this, callfunc_selector(pikeMan::attack_A_img_end));
	CCActionInterval* attackact = CCSequence::create(act, callFunc, NULL);//序列动作
	m_Sprite->runAction(attackact);
}
void pikeMan::attack_A_img_end(){
	...
	//将动画包装成一个动作  
	Animate* act = Animate::create(animation);
	CCCallFunc *callFunc = CCCallFunc::create(CC_CALLBACK_0(pikeMan::attack_A_end, this));
	CCActionInterval* attackact = CCSequence::create(act, callFunc, NULL);//序列动作
	m_Sprite->runAction(attackact);
}
void pikeMan::attack_A_end(){//停止攻击
	if (SpriteDirecton){//移动间攻击
...
void pikeMan::attack_A_effect(AllSprites *enemy){//溅血效果
...
void pikeMan::blood_effect(AllSprites *sprite){//溅血效果
	Vector<SpriteFrame* > cache;//缓冲池
	char txt[50] = {};
	int i;
	for (i = 0; i <= 4; i++)
	...
	Action * act = Sequence::create(
		...
		CCCallFuncN::create(sprite->get_n_Sprite(), callfuncN_selector(pikeMan::removeself)), //删除自己
		NULL);
...
void pikeMan::removeself(Node *pSender){//从父类移除图片特效
	pSender->removeFromParentAndCleanup(true);
}
void pikeMan::move(){//朝着目标移动
	isMoving = true;
...
void pikeMan::deadth_ani(){
	...
	IsRunning = true;
	CCCallFunc* callFunc = CCCallFunc::create(this, callfunc_selector(pikeMan::setisOver));
	CCActionInterval* deathact = CCSequence::create(act, fadeout, callFunc, NULL);//序列动作
	m_Sprite->runAction(deathact);
}
void pikeMan::setisOver(){
	isOver = true;
}
void pikeMan::SetAnimation(int from, int to, bool run_directon, int loop){//动画
...
void pikeMan::StopAnimation(){//停止所有动画
	if (!IsRunning)
		return;
	m_Sprite->stopAllActions();//当前精灵停止所有动画  
	//恢复精灵原来的初始化贴图   
...
void pikeMan::walk(bool run_directon){
	this->SetAnimation(walk_from, walk_to, run_directon, -1);
}

bool pikeMan::get_skill_J_ishurted(){
	return skill_J_ishurted;
}
void pikeMan::set_skill_J_ishurted(bool skill_J_ishurted){
	this->skill_J_ishurted = skill_J_ishurted;
}
bool pikeMan::get_skill_K_ishurted(){
	return skill_K_ishurted;
}
void pikeMan::set_skill_K_ishurted(bool skill_K_ishurted){
	this->skill_K_ishurted = skill_K_ishurted;
}
int pikeMan::get_life(){
	return life;
}
void pikeMan::set_life(int delta){
	life += delta;
	auto act = ScaleTo::create(0.2f, life / (float)orginal_life*0.4, 0.7);
	life_bar_fore->runAction(act);
}
void pikeMan::set_life_bar(int side){
	if (side == 0){
...
bool pikeMan::get_hittest_A(){
	return hittest_A;
}
void pikeMan::set_hittest_A(bool hittest_A){
	this->hittest_A = hittest_A;
}
bool pikeMan::get_Direction(){
	return SpriteDirecton;
}
bool pikeMan::get_isblowup(){
	return isblowup;
}
void pikeMan::set_isblowup(bool isblowup){
	this->isblowup = isblowup;
}
bool pikeMan::get_canControl(){
	return canControl;
}
void pikeMan::set_canControl(bool canControl){
	this->canControl = canControl;
}
bool pikeMan::get_isOver(){
	return isOver;
}
bool pikeMan::get_isdeath(){
	return isdeath;
}
int pikeMan::get_typeid(){
	return type_id;
}
int pikeMan::get_attack_A_distance(){
	return attack_area;
}
bool pikeMan::is_melee(){
	return is_melee_unit;
}
float pikeMan::get_speed(){
	return speed;
}
int pikeMan::earn_money(){
	return pay_back;
}
Sprite* pikeMan::get_n_Sprite(){
	return	n_Sprite;
}
void pikeMan::set_money_paid(bool paid){
	this->paid = paid;
}
bool pikeMan::get_money_paid(){
	return paid;
}
void pikeMan::set_isMoving(bool isMoving){
	this->isMoving = isMoving;
}
bool pikeMan::get_isMoving(){
	return isMoving;
}