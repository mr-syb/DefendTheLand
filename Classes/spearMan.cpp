...
void spearMan::object_select(Vector<AllSprites *> sprites){//判定攻击目标
...
void spearMan::attack_ani(){//攻击
	...
	if (m_Sprite->getAnchorPoint().x != 0.125&&m_Sprite->getAnchorPoint().y != 0.125){//此段代码解决了点击普攻闪烁问题
		...
		m_Sprite = Sprite::createWithSpriteFrameName("A4f_0.png");//恢复精灵原来的贴图样子
		m_Sprite->setFlipX(SpriteDirecton);
...
void spearMan::attack_A_end(){//停止攻击
	if (SpriteDirecton){//移动间攻击
...
void spearMan::attack_A_effect(AllSprites *enemy){//溅血效果
...
void spearMan::blood_effect(AllSprites *sprite){//溅血效果
	Vector<SpriteFrame* > cache;//缓冲池
	char txt[50] = {};
	int i;
	for (i = 0; i <= 4; i++)
	...
	Action * act = Sequence::create(
		...
		CCCallFuncN::create(sprite->get_n_Sprite(), callfuncN_selector(spearMan::removeself)), //删除自己
		NULL);
...
void spearMan::removeself(Node *pSender){//从父类移除图片特效
	pSender->removeFromParentAndCleanup(true);
}
void spearMan::move(){//朝着目标移动
	isMoving = true;
...
void spearMan::SetAnimation(int from, int to, bool run_directon, int loop){//动画
...
void spearMan::StopAnimation(){//停止所有动画
	if (!IsRunning)
		return;
	m_Sprite->stopAllActions();//当前精灵停止所有动画  
	//恢复精灵原来的初始化贴图   
